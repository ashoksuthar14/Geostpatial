...
function showPosition(position) {
    ...
    window.streamlitApi.runScript({
        "script": `st.session_state.lat = ${lat}; st.session_state.lng = ${lng};`
    });
}
...
# Check if location is available in session state
if "lat" in st.session_state and "lng" in st.session_state:
    ...
    if st.button("Find Hospitals"):
        if lat is not None and lng is not None:
            ...
            st.write("Unable to detect your location. Please ensure location access is enabled.")
else:
    st.write("Unable to detect your location. Please ensure location access is enabled.")
